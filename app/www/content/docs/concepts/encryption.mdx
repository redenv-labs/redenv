...
description: Understand Redenv's zero-knowledge encryption model, how your secrets are protected, and the cryptographic primitives used.
---
...
1. **Use read-only tokens** when possible — limits blast radius if leaked
2. **Rotate tokens regularly** — create new, revoke old
3. **One token per environment** — separate dev, staging, prod
4. **Store tokens securely** — use platform secrets (GitHub Secrets, Vercel env vars), not code
...
- [Service Tokens](/docs/concepts/tokens) — how tokens work and management best practices
- [CLI Commands](/docs/cli/commands) — full reference for all Redenv commands
- [Version History](/docs/concepts/versioning) — encrypted historical versions
